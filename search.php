		...
		<div id="main">
			...
			<article class="post">
				...
				if (!empty($_GET['search'])) {
					$search = mysqli_real_escape_string($conn, htmlspecialchars($_GET['search']));
					$query = "SELECT P.title, P.id, P.author, P.postdate, P.image, P.content, P.tag, P.status, P.cat_id, C.cat_title 
								FROM posts AS P JOIN categories AS C on P.cat_id = C.cat_id WHERE status='published' AND tag LIKE '%" . $search . "%' ORDER BY updated_on DESC";
				} else if (!empty($_GET['category'])) {
					$cat_title = mysqli_real_escape_string($conn, htmlspecialchars($_GET['category']));
					$query = "SELECT P.title, P.id, P.author, P.postdate, P.image, P.content, P.tag, P.status, P.cat_id, C.cat_title 
								FROM posts AS P JOIN categories AS C on P.cat_id = C.cat_id WHERE status='published' AND cat_title='" . $cat_title . "' ORDER BY updated_on DESC";
				} else if (!empty($_GET['author'])) {
					$author = mysqli_real_escape_string($conn, htmlspecialchars($_GET['author']));
					$query = "SELECT P.title, P.id, P.author, P.postdate, P.image, P.content, P.tag, P.status, P.cat_id, C.cat_title 
								FROM posts AS P JOIN categories AS C on P.cat_id = C.cat_id WHERE status='published' AND author='" . $author . "' ORDER BY updated_on DESC";
				} else {
